...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for(int i=0;i<word.length();i++) {
			...
			if(isVowel(ch))	{
				...
				else if(i == word.length()-1 && ch == 'e')
					break;
				else if(i != 0 && i == word.length()-1 && ch != 'e') {
					if(!isPreviousLetterVowel(Character.toLowerCase(word.charAt(i-1)))) {
						count++;
					}
				}
				else if (i != 0 && !isPreviousLetterVowel(Character.toLowerCase(word.charAt(i-1)))) {
					count++;
				}
			}
		}
	...
	public boolean isPreviousLetterVowel(char prev) {
		if(isVowel(prev))
			return true;
		return false;
	}
